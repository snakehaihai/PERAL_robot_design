...
```ros2 launch msg_MID360_launch.py``` publishes ```CustomMsg``` and ```PointCloud2``` messages simultaneously.
...
Launch files of ROS are in the "[workspace]/src/livox_ros_driver2/src/launch" directory. Different launch files have different configuration parameter values and are used in different scenarios:
