...
void loop() {
    ...
    if (millis() - last_control_time > control_interval) {
        ...
        // Calculate current wheels velocity in rad/s
        left_velocity = calculate_velocity(delta_left, delta_time, L_ENCODER_CPR);
        right_velocity = calculate_velocity(delta_right, delta_time, R_ENCODER_CPR);

        // Publish wheels velocity via micro-ROS
        publish_wheels_velocity(left_velocity, right_velocity);

        // Calculate angular velocity (rad/s) of each wheel given linear_vel and angular_vel of the robot
        left_target_velocity  = (linear_vel - (angular_vel * WHEEL_SEPARATION) / 2) / WHEEL_RADIUS;
        right_target_velocity = (linear_vel + (angular_vel * WHEEL_SEPARATION) / 2) / WHEEL_RADIUS;

        // Compute PID outputs
        int left_pwm = left_pid.compute(left_target_velocity, left_velocity);
        int right_pwm = right_pid.compute(right_target_velocity, right_velocity);
        
        // Apply motor control
        set_motor_pwm(PWMA, AIN1, AIN2, left_pwm);
        set_motor_pwm(PWMB, BIN2, BIN1, right_pwm);

        // For debugging
        // Serial.printf("Left Error: %.2f | Left PWM: %d | Left Encoder: %d | Left: %.2f | Right Error: %.2f | Right PWM: %d | Right Encoder: %d | Right: %.2f\n",
        //         left_pid.getError(), left_pwm, left_encoder_count, left_velocity, right_pid.getError(), right_pwm, right_encoder_count, right_velocity);
    }
}