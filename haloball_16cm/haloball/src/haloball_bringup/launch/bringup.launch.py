...
def generate_launch_description() -> LaunchDescription:
    ...
    # Perform transformation from odom -> base_link and publish /lidar_odometry
    base_link_tf_node = Node(
        ...
        parameters=[
            {'lidar_to_base_link_distance': 0.006}, # default value is 0.0m, my base_link is center of ball to utilise similar triangle property
        ]
    )
    
    # Links odom and camera_init frames
    tf_adapter = Node(
        package="tf2_ros",
    ...
    pointcloud_filter_config = os.path.join(get_package_share_directory(package_name), 'config', 'pointcloud_filter.yaml')
    # Launch pointcloud_filter_node
    pointcloud_filter_node = Node(
        package=package_name,
        executable='pointcloud_filter',
        name='pointcloud_filter',
        parameters=[pointcloud_filter_config]
    )
    ...
    laser_to_pointcloud_node = Node(
        ...
        remappings=[
            # Remap the default '/topic' to '/your_topic'
            ('/scan', '/scan'),
            ('/scan_pointcloud', '/scan_pointcloud'),
        ],
    )
    ...
    ld.add_action(tf_adapter)
    ld.add_action(rviz2_node)

    # ld.add_action(pointcloud_filter_node)
    # ld.add_action(laser_to_pointcloud_node)
